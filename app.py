...
def tria_candidat(df: pd.DataFrame, assigned: set, estranger_count: int, assignats: int, vedat: str | None, assignats_parr: dict, rng: np.random.RandomState) -> int | None:
...
def sorteig_individual(df: pd.DataFrame, tipus_quant: list[tuple[str, int]], ordre_aleatori: bool, vedat: str | None, rng: np.random.RandomState) -> pd.DataFrame:
    """Realitza el sorteig per una unitat de gestió."""
    df = df.copy()
        ...
        for tipus, q in tipus_quant:
            for _ in range(q):
                ...
                if vedat and df.at[idx, "Parroquia"] in assignats_parr:
                    assignats_parr[df.at[idx, "Parroquia"]] += 1
                ordre += 1
    col = vedat if vedat else ""
    res = df[["ID", "ordre", "tipus", "Estranger"]].copy()
    return res


def processar_sorteigs(df1: pd.DataFrame, df2: pd.DataFrame, config: pd.DataFrame, especie: str, ordre_aleatori: bool, seed: int | None) -> tuple[pd.DataFrame, list[pd.DataFrame]]:
    ...
    for sorteig in ESPECIE_SORTEIGS[especie]:
        ...
        asignats = sorteig_individual(part, tipus_quant, ordre_aleatori, vedat, rng)
...
with st.expander("Configuració de captures per sorteig"):
    initial_rows = [{"Codi_Sorteig": c, "Tipus": [], "Quantitat": 0} for c in ESPECIE_SORTEIGS[especie]]
    config_df = st.data_editor(
        pd.DataFrame(initial_rows),
        column_config={
            "Tipus": st.column_config.ListColumn("Tipus", width="medium", default=[], options=TIPUS_OPTIONS),
...
ordre_aleatori = st.radio(
    "Ordre d'assignació de captures", ["En l’ordre fixat", "Ordre aleatori"],
    horizontal=True
) == "Ordre aleatori"
...
if st.button("Executar sorteig"):
    ...
    try:
        ...
        st.error(str(e))
        st.stop()
    resultat, resums = processar_sorteigs(df1, df2, config_df, especie, ordre_aleatori, seed)
